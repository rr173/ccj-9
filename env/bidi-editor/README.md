...
点击工具栏 **🧪 双向安全实验室** 打开。实验室只读诊断正文，修复必须显式勾选并确认。

### 载入

| 入口 | 行为 |
|---|---|
| **① 载入当前编辑内容** | 复制编辑器全部段落（文本+方向）进入实验室；诊断全过程不触碰原文 |
| **载入手动样例** | 文本框每行一段，行首可加 `[auto]`/`[ltr]`/`[rtl]` 指定方向；独立样例不接触正文 |
| 样例“载入实验室” | 从已保存的诊断样例载入（回归专用） |

单段超过 5 万码点会被拒绝载入并提示拆分；超过 5000 码点只诊断前 5000 码点并给出截断提示。

### 诊断（③）

逐段检查并报告：不可见方向标记、**未配对的嵌入与隔离控制符**、**跨段落方向状态**
（段落边界强制重置方向栈）、旧式嵌入（LRE/RLE）、强制覆盖（LRO/RLO）、嵌套过深
（>60 预警，>125 静默忽略）、数字体系混用（同串 EN+AN）、数字/标点方向歧义、
中立符夹在两套文字之间、括号未配对、悬空 ZWJ/ZWNJ、覆盖作用域内的视觉/逻辑可疑片段。

每个问题展示：

- **段落号**（1 起）与**逻辑码点范围**（半开 `[start,end)`）
- 每个码点的 **Unicode 码点名称**（内置精确名称表+区段规则名；无数据时诚实显示
  `UNNAMED CHARACTER xxxx`，不编造名称）
- **所属字素簇**（含组合字符、代理对、ZWJ 序列的簇整体展示，绝不拆成多个可修复位置）
- 风险原因；阿拉伯文 RTL 显示时所有位置仍按原始逻辑文本编号

### 双视图与三向定位

- **逻辑序列**：内存中码点顺序，每格标注逻辑起始码点
- **实际渲染序列**：把段落放进与条件同宽度/字号/字体/方向的离屏舞台，
  按浏览器实际字形位置（行自上而下、行内按 x）测量，折行单独成行
- 点击任一视图中的字符或问题项：另一视图自动滚动到**同一个字素簇**并高亮，
  同时在原编辑器中定位（仅当正文未变化；已变化则明确提示）

改变段落方向（工具栏或实验室内方向按钮）、容器宽度、缩放比例或字体后，
映射立即标记 **⚠ 旧诊断已过期**、应用按钮禁用，必须重新诊断才能修复——
不能拿旧位置执行修复。
...
每条建议可打开**预览**：修复前后的逻辑码点对照表、同宽度下的实际渲染预览、
以及高亮标注的**可能改变的视觉范围**。

勾选多条后 **④ 批量应用**：

- 应用前比对**正文指纹**与**渲染条件指纹**（段落方向/宽度/缩放/字体），
  任一变化**整次拒绝、不改任何文字**，并标记诊断过期
- 同段建议范围重叠、同段两个方向调整，同样整次拒绝
- 成功后实验室副本同步、立即重算映射，并回到原编辑器中修复对应的位置

### 一次性撤销

应用时同步捕获修复前的完整状态（文本、段落方向、选区、滚动位置）。
撤销**精确还原**全部四项；若修复后正文又被修改，撤销被拒绝以避免覆盖新编辑。
撤销记录一次性使用：重复撤销明确拒绝。

### 诊断样例与回归

- 可把当前实验室内容（自动附带诊断到的问题与视觉锚点）保存为**诊断样例**：
  记录预期问题类型、逻辑位置与视觉锚点
- **批量重新检查**对每个样例报告：**新增 / 消失 / 位置变化 / 渲染顺序变化**
  （视觉锚点指纹比对），并给出汇总计数；回归全程只读，**绝不改动编辑器正文**

样例、诊断报告、修复记录与撤销结果通过服务端持久化（`data/bidi-lab.json`，
`BIDI_LAB_FILE` 覆盖），刷新后仍可查看；集合采用独立版本号 `X-Bidi-Lab-Rev`，
变更请求须 `If-Match` 严格相等。保存失败时明确提示，当前选择与预览内容保留。

### HTTP API 摘要

```
GET    /api/bidi-lab/samples             样例列表（X-Bidi-Lab-Rev）
POST   /api/bidi-lab/samples             新建样例（If-Match，空名/超长/坏段落 4xx，重名 409）
GET    /api/bidi-lab/samples/:id         样例详情（404）
PUT    /api/bidi-lab/samples/:id         更新样例（If-Match）
DELETE /api/bidi-lab/samples/:id         删除样例（If-Match）
POST   /api/bidi-lab/samples/recheck     批量回归（只读，可选 {ids}，不推进 rev）
GET    /api/bidi-lab/records[?kind=]     报告/修复/撤销记录（按时间倒序）
POST   /api/bidi-lab/records             保存记录（If-Match；undo 须对应 repair，重复撤销 409）
```

纯逻辑在 `bidi-lab-core.js`（无 DOM 依赖，浏览器/Node 共用），实际字形测量在
`bidi-lab-geometry.js`，UI 在 `bidi-lab.js`。
...
双向安全实验室数据（样例/报告/修复/撤销记录）默认写到 `./data/bidi-lab.json`（`BIDI_LAB_FILE` 覆盖，集合版本号为响应头 `X-Bidi-Lab-Rev`）。
...
- `test/bidi-lab-core.test.js`：字素簇切分（组合字符/代理对/ZWJ/国旗与 emoji 序列不拆散）、码点名称诚实兜底、UAX #9 重排（RTL 数字保序、括号镜像、嵌入隔离配对）、逐段诊断（孤立/未关闭/跨段/旧式/覆盖/深嵌套/混用数字/标点歧义/中立符/括号/悬空连接符）、逻辑位置不随 RTL 反转、修复计划、正文/渲染指纹门禁（变化整次拒绝）、范围重叠拒绝、样例校验与回归（新增/消失/移位/渲染顺序变化）
- `test/bidi-lab-api.test.js`：真实起服务跑样例 CRUD、乐观锁 428/409、重名、批量回归只读、记录校验、撤销对应修复与重复撤销拒绝、刷新后持久化
...
bidi-lab-core.js 双向安全实验室纯逻辑（浏览器与 Node 共用，无 DOM 依赖）：UAX #29 扩展字素簇切分（Intl.Segmenter 优先，状态机兜底）、码点名称表、UAX #9 简化重排（嵌入/隔离/覆盖栈、数字规则、中立符解析、括号镜像、级别反转）、逐段诊断、修复计划、正文/渲染条件指纹门禁与整批应用、样例校验与回归比对
bidi-lab-geometry.js 实验室实际渲染序列测量（仅浏览器）：离屏舞台逐字素簇测量 getClientRects，按行与 x 得到浏览器真实阅读顺序；字体可用性检测
bidi-lab.js       双向安全实验室 UI：载入、渲染条件、逻辑/渲染双视图三向定位、问题清单（段落号/逻辑码点范围/码点名称/字素簇/原因）、过期标记、修复预览（前后码点/同宽度渲染/视觉范围）、组合批量应用、一次性撤销、样例保存与批量回归、历史记录（window.BidiLabUI）
